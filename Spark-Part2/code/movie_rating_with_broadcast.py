...
def load_movie_name():
    file_path = "Spark-Part2/dataset/movies.txt"
    with open(file_path, "r") as f:
        all_movies = f.readlines()
    movie_names = dict()
    for i in all_movies:
        each_movie = i.split("::")
        movie_names[int(each_movie[0])] = each_movie[1]
    return movie_names

movie_names = sc.broadcast(load_movie_name())
# Lets join this movie_names_rdd_1 and movie_id_rating_more_5 
movie_names_rdd_2 = movie_id_rating_more_5.map(lambda x: (movie_names.value[x[0]], x[1][1]))
